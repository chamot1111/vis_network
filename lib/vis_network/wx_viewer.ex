defmodule VisNetwork.WxViewer do
  @moduledoc false
  ...
  cond do
    not Code.ensure_loaded?(:wx) ->
      def start(_html) do
        raise RuntimeError,
              "VisNetwork.Viewer requires Erlang compilation to include the :wx module, but it's not available"
      end

    not Code.ensure_loaded?(:wxWebView) ->
      def start(_html) do
        raise RuntimeError,
              "VisNetwork.Viewer requires Erlang compilation to include the :wxWebView module, but it's not available." <>
